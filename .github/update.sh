...
update_version() {
  ...
  if [ "$version_name" = "twilight" ]; then
    ...
    flake_repo_location="wuz/zen-browser-flake"
    ...
    get_twilight_release_artifact_meta_from_zen_repo "$arch" "$os" |
      while read -r line; do
        ...
        if [ "$os" = "darwin" ]; then
          self_download_url="https://github.com/wuz/zen-browser-flake/releases/download/$release_name/zen.macos-universal.dmg"
        else
          self_download_url="https://github.com/wuz/zen-browser-flake/releases/download/$release_name/zen.linux-$arch.tar.xz"
        fi
